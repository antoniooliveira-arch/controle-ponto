...
function CreateEmployeeForm({ onCreated }: { onCreated: () => void }) {
  ...
  const [fullName, setFullName] = useState(""); const [registration, setRegistration] = useState(""); const [sectorId, setSectorId] = useState(""); const [funcao, setFuncao] = useState(""); const [cargo, setCargo] = useState(""); const [lotacaoLocal, setLotacaoLocal] = useState(""); const [cargaHoraria, setCargaHoraria] = useState("8h"); const [password, setPassword] = useState(""); const [confirmPassword, setConfirmPassword] = useState(""); const [newSector, setNewSector] = useState("");
  useEffect(() => { if (sectors.data && !sectorId) { const nise = sectors.data.find(s => s.name.trim().toUpperCase() === "NISE"); if (nise) setSectorId(String(nise.id)); } }, [sectors.data, sectorId]);
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (password !== confirmPassword) return toast.error("A confirmação de senha não corresponde."); create.mutate({ fullName, registration, sectorId: sectorId ? Number(sectorId) : null, funcao, cargo, lotacaoLocal, cargaHoraria, password }); };
  return <form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2 sm:col-span-2"><Label htmlFor="new-name" className="tiny-label">Nome completo</Label><Input id="new-name" value={fullName} onChange={event => setFullName(event.target.value)} required className="admin-input" /></div><div className="space-y-2"><Label htmlFor="new-reg" className="tiny-label">Matrícula</Label><Input id="new-reg" value={registration} onChange={event => setRegistration(event.target.value)} required className="admin-input" /></div><div className="space-y-2"><Label htmlFor="new-sector" className="tiny-label">Setor</Label><select id="new-sector" value={sectorId} onChange={event => setSectorId(event.target.value)} className="admin-select"><option value="">Sem setor</option>{sectors.data?.filter(sector => sector.active).map(sector => <option key={sector.id} value={sector.id}>{sector.name}</option>)}</select></div><div className="space-y-2"><Label htmlFor="new-funcao" className="tiny-label">Função</Label><Input id="new-funcao" value={funcao} onChange={event => setFuncao(event.target.value)} className="admin-input" /></div><div className="space-y-2"><Label htmlFor="new-cargo" className="tiny-label">Cargo</Label><Input id="new-cargo" value={cargo} onChange={event => setCargo(event.target.value)} className="admin-input" /></div><div className="space-y-2"><Label htmlFor="new-lotacao" className="tiny-label">Lotação / Local</Label><Input id="new-lotacao" value={lotacaoLocal} onChange={event => setLotacaoLocal(event.target.value)} className="admin-input" /></div><div className="space-y-2"><Label htmlFor="new-carga" className="tiny-label">Carga horária</Label><Input id="new-carga" value={cargaHoraria} onChange={event => setCargaHoraria(event.target.value)} className="admin-input" /></div><div className="space-y-2"><Label htmlFor="new-password" className="tiny-label">Senha inicial</Label><Input id="new-password" value={password} onChange={event => setPassword(event.target.value)} type="password" minLength={4} required className="admin-input" /></div><div className="space-y-2"><Label htmlFor="confirm-password" className="tiny-label">Confirmar senha</Label><Input id="confirm-password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} type="password" minLength={4} required className="admin-input" /></div></div><div className="flex flex-wrap items-center gap-2 border-t border-stone-900/10 pt-4"><Input value={newSector} onChange={event => setNewSector(event.target.value)} placeholder="Novo setor" className="h-9 max-w-[185px] rounded-none border-stone-900/20 text-sm" /><Button type="button" variant="outline" onClick={() => { if (newSector.trim()) createSector.mutate({ name: newSector }); }} disabled={createSector.isPending} className="h-9 rounded-none border-stone-900/25 text-xs">Adicionar setor</Button></div><Button type="submit" disabled={create.isPending} className="h-11 w-full rounded-none bg-stone-950 text-xs tracking-[0.15em]">{create.isPending ? <Loader2 className="animate-spin" /> : "Salvar servidor"}</Button></form>;
}
...
function EditEmployeeDialog({ employee, onClose, onSave, isSaving }: { employee: EmployeeItem | null; onClose: () => void; onSave: (value: { employeeId: number; fullName: string; registration: string; sectorId: number | null; funcao: string | null; cargo: string | null; lotacaoLocal: string | null; cargaHoraria: string | null; active: boolean }) => void; isSaving: boolean }) {
  const sectors = trpc.admin.sectors.useQuery();
  const [fullName, setFullName] = useState(""); const [registration, setRegistration] = useState(""); const [sectorId, setSectorId] = useState(""); const [funcao, setFuncao] = useState(""); const [cargo, setCargo] = useState(""); const [lotacaoLocal, setLotacaoLocal] = useState(""); const [cargaHoraria, setCargaHoraria] = useState(""); const [active, setActive] = useState(true);
  const sync = (open: boolean) => { if (open && employee) { setFullName(employee.fullName); setRegistration(employee.registration); setSectorId(employee.sectorId ? String(employee.sectorId) : ""); setFuncao(employee.funcao ?? ""); setCargo(employee.cargo ?? ""); setLotacaoLocal(employee.lotacaoLocal ?? ""); setCargaHoraria(employee.cargaHoraria ?? "8h"); setActive(employee.active); } if (!open) onClose(); };
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (employee) onSave({ employeeId: employee.id, fullName, registration, sectorId: sectorId ? Number(sectorId) : null, funcao, cargo, lotacaoLocal, cargaHoraria, active }); };
  return <Dialog open={Boolean(employee)} onOpenChange={sync}><DialogContent className="rounded-none bg-[#f8f3e9] sm:max-w-lg"><DialogHeader><p className="tiny-label">Gestão de cadastro</p><DialogTitle className="font-serif text-3xl">Editar servidor</DialogTitle></DialogHeader><form onSubmit={submit} className="space-y-4"><div className="space-y-2"><Label className="tiny-label">Nome completo</Label><Input value={fullName} onChange={event => setFullName(event.target.value)} required className="admin-input" /></div><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label className="tiny-label">Matrícula</Label><Input value={registration} onChange={event => setRegistration(event.target.value)} required className="admin-input" /></div><div className="space-y-2"><Label className="tiny-label">Setor</Label><select value={sectorId} onChange={event => setSectorId(event.target.value)} className="admin-select"><option value="">Sem setor</option>{sectors.data?.map(sector => <option value={sector.id} key={sector.id}>{sector.name}</option>)}</select></div></div><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label className="tiny-label">Função</Label><Input value={funcao} onChange={event => setFuncao(event.target.value)} className="admin-input" /></div><div className="space-y-2"><Label className="tiny-label">Cargo</Label><Input value={cargo} onChange={event => setCargo(event.target.value)} className="admin-input" /></div><div className="space-y-2"><Label className="tiny-label">Lotação / Local</Label><Input value={lotacaoLocal} onChange={event => setLotacaoLocal(event.target.value)} className="admin-input" /></div><div className="space-y-2"><Label className="tiny-label">Carga horária</Label><Input value={cargaHoraria} onChange={event => setCargaHoraria(event.target.value)} className="admin-input" /></div></div><label className="flex items-center justify-between border-y border-stone-900/10 py-4"><span><strong className="text-sm">Acesso do servidor</strong><span className="mt-1 block text-xs text-stone-500">Desativar encerra as sessões existentes.</span></span><button type="button" onClick={() => setActive(value => !value)} className={`toggle-control ${active ? "toggle-control-on" : ""}`} aria-pressed={active}><span /></button></label><Button type="submit" disabled={isSaving} className="h-11 w-full rounded-none bg-stone-950 text-xs tracking-[0.15em]">{isSaving ? <Loader2 className="animate-spin" /> : active ? "Salvar alterações" : "Bloquear acesso"}</Button></form></DialogContent></Dialog>;
}
